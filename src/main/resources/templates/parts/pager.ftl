...
<#macro qualifiedPager url page qualifier submitButtonId>
<#if page.getTotalPages() gt 4>
    <#assign
    totalPages = page.getTotalPages()
    pageNumber = page.getNumber() + 1
...
<div class="mt-2">
    <ul class="pagination">
        ...
        <#else>
        <li class="page-item">
            <a class="page-link" href="#" role="button" tabindex="-1" id="${qualifier}page${p}">${p}</a>
            <script>
                    page = document.getElementById('${qualifier}page${p}')
                    page.addEventListener('click', ()=>{setValueById('${qualifier}PageNumber', '${p-1}')})
                    page.addEventListener('click', ()=>{setValueById('${qualifier}PageSize', '${page.getSize()}')})
                    page.addEventListener('click', ()=>{clickSubmitButton('${submitButtonId}')})
            </script>
        </li>
    </#if>
    ...
    <input name="${qualifier}_page" id="${qualifier}PageNumber" value="${pageNumber}" hidden>
    <input name="${qualifier}_size" id="${qualifier}PageSize" value="${page.getSize()}" hidden>
</div>
</#macro>
